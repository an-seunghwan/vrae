...
PARAMS = {
    ...
    "epsilon_std": 0.01,
    "anneal_rate": 0.0003,
    "init_temperature": 4.0, # shared temperature
    "min_temperature": 0.1,
    "learning_rate": 0.005,
    "stable": False
}
...
class CDVAE(K.models.Model):
    def __init__(self, params):
        ...
        self.dec_dense2 = layers.Dense(params["data_dim"], activation='sigmoid')
...
mean, logvar, logits, z, xhat = model(x_train[:PARAMS['batch_size']], tau=2.0)
...
plt.savefig('./result/vae_embed1.png')
#%%
'''discrete'''
...
for i in range(0, 1000):
    plt.scatter(viz[i, 0], viz[i, 1], c=color[y[i]])
plt.savefig('./result/vae_embed_discrete1.png')
...
for i in range(10):
    ...
    plt.axis('off')
    plt.tight_layout() 
plt.savefig('./result/vae_embed_discrete2.png')
#%%
'''continuous'''
C = np.zeros((10000, PARAMS["class_num"]))
for i in range(0, 10000, PARAMS['batch_size']):
...
for i in range(0, 1000):
    plt.scatter(viz[i, 0], viz[i, 1], c=color[y[i]])
plt.savefig('./result/vae_embed_continuous1.png')
#%%