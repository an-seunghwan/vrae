...
def loss_fun(x, xhat, mean, logvar, logits, beta):
    ...
    logits_ = logits / tf.reduce_sum(logits)
    disc_kl = tf.reduce_mean(tf.reduce_sum(logits_ * (tf.math.log(logits_ + 1e-20) - tf.math.log(1/PARAMS['class_num'])), axis=1), keepdims=True)
...
def save_plt(original_img, construct_img, code):
    ...
    plt.savefig('./result/vae_rebuilt_200903.png')
...
for i in range(0, 10000, PARAMS['batch_size']):
    _, _, _, z, _ = model(x_test[i: i+PARAMS['batch_size']], tau=0.1)
...
plt.savefig('./result/vae_embed_200903.png')
#%%
'''discrete'''
...
for i in range(0, 1000):
    plt.scatter(viz[i, 0], viz[i, 1], c=color[y[i]])
plt.savefig('./result/vae_embed_discrete1_200903.png')
...
for i in range(10):
    ...
    plt.axis('off')
    plt.tight_layout() 
plt.savefig('./result/vae_embed_discrete2_200903.png')
#%%
'''continuous'''
C = np.zeros((10000, PARAMS["class_num"]))
for i in range(0, 10000, PARAMS['batch_size']):
...
for i in range(0, 1000):
    plt.scatter(viz[i, 0], viz[i, 1], c=color[y[i]])
plt.savefig('./result/vae_embed_continuous_200903.png')
#%%