...
class GumbelSoftmaxVAE(K.models.Model):
    def __init__(self, params):
        ...
        # encoder
        self.enc_dense1 = layers.Dense(512, activation='linear')
        self.enc_dense2 = layers.Dense(256, activation='linear')
...
def gumbel_loss(logits_y, logits_x, x):
    ...
    # cross-entropy
    # cross_ent = K.losses.binary_crossentropy(x, logits_x, from_logits=False)
...
def get_learning_rate(init=PARAMS["learning_rate"]):
    ...
    return tf.convert_to_tensor(init * 0.9, dtype=tf.float32)
#%%
# data
...
for epoch in range(1, PARAMS["epochs"] + 1):
    ...
    train_x = next(iter(train_dataset))
...
logits_y, logits_x = model(x_train[:PARAMS['batch_size']], tau=1.0)
...
# code = model.sample_y
save_plt(x_train[:PARAMS['batch_size']], logits_x, sample_y)
#%%
'''t-SNE'''
# def save_embed():
