...
class GumbelSoftmaxMixtureVAE(K.models.Model):
    def __init__(self, params):
        ...
        # discrete latent
        self.logits = layers.Dense(self.params["class_num"], activation='sigmoid') # non-negative logits

        # decoder
        self.dec_dense1 = layers.Dense(512, activation='tanh')
        self.dec_dense2 = layers.Dense(256, activation='tanh')
...
def gumbel_loss(logits, xhat, x, mean, logvar, temperature, beta):
    ...
    kl1 = tf.reduce_sum(logits_ * (tf.math.log(logits_ + 1e-20) - tf.math.log(1/PARAMS['class_num'])), axis=-1)
    sampled_z = model.gaussian_sample()
    posterior_pdf = gaussian_density(sampled_z, mean, logvar)
    prior_pdf = gaussian_density(sampled_z, prior_means, tf.ones(logvar.shape))
    kl2 = tf.math.log(tf.reduce_sum(logits_ * posterior_pdf, axis=-1) / tf.reduce_mean(prior_pdf, axis=-1) + 1e-20)
    kl_loss = tf.reduce_mean(kl1 + kl2)
...
def save_plt(original_img, construct_img, code):
    ...
    for i in range(0, 15, 3):
        ...
        plt.axis('off')
    if PARAMS['FashionMNIST']:
        plt.savefig('./result/mixturevae_fashionmnist_rebuilt_200904.png')
    else:
        plt.savefig('./result/mixturevae_mnist_tsne_200904.png')

mean, logvar, logits, z, z_tilde, xhat= model(x_train[:PARAMS['batch_size']], tau)
...
for i in range(PARAMS['class_num']):
    ...
    plt.tight_layout() 
if PARAMS['FashionMNIST']:
    plt.savefig('./result/mixturevae_fashionmnist_center_200904.png')
else:
    plt.savefig('./result/mixturevae_mnist_center_200904.png')
#%%
'''t-SNE'''
C = np.zeros((10000, PARAMS["latent_dim"]))
for i in range(0, 10000, PARAMS['batch_size']):
    _, _, _, _, z_tilde, _ = model(x_test[i: i+PARAMS['batch_size']], tau)
...
if PARAMS['FashionMNIST']:
    plt.savefig('./result/mixturevae_fashionmnist_tsne_200904.png')
else:
    plt.savefig('./result/mixturevae_mnist_tsne_200904.png')
#%%