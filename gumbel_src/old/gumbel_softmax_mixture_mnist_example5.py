...
np.random.seed(10)
# prior_means = np.tile(np.random.uniform(low=-2, high=2, size=(PARAMS['class_num'], PARAMS['latent_dim'])), (PARAMS['batch_size'], 1, 1))
...
for epoch in range(1, PARAMS["epochs"] + 1):
    ...
    for train_x, train_y in train_dataset:
        with tf.GradientTape(persistent=True) as tape:
            ...
            loss1, mse, kl_loss = gumbel_loss(logits, xhat, train_x, mean, logvar, tau, beta)
            loss2 = scce(train_y, logits)
            loss = loss1 + loss2
