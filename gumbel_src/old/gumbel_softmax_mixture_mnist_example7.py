...
# np.random.seed(10)
# prior_means = np.tile(np.random.uniform(low=-2.5, high=2.5, size=(PARAMS['class_num'], PARAMS['latent_dim'])), (PARAMS['batch_size'], 1, 1))
...
prior_means[:PARAMS['class_num'], :PARAMS['class_num']] = np.eye(PARAMS['class_num']) * 2
prior_means = np.tile(prior_means, (PARAMS['batch_size'], 1, 1))
...
class GumbelSoftmaxMixtureVAE(K.models.Model):
    def __init__(self, params):
        ...
        # discrete latent
        self.logits = layers.Dense(self.params["class_num"], activation='tanh') # non-negative logits
    ...
    def call(self, x, tau):
        ...
        # discrete latent
        logits = tf.math.exp(self.logits(x))
...
# linear
# def kl_anneal(epoch, s, k=PARAMS['gamma']):
#     return (k / s) * epoch
...
for epoch in range(1, PARAMS["epochs"] + 1):
    ...
    # KL annealing
    # beta = kl_anneal(epoch, int(PARAMS["epochs"]/2)) * PARAMS['gamma']

    for train_x in train_dataset:
        with tf.GradientTape(persistent=True) as tape:
            mean, logvar, logits, z, z_tilde, xhat = model(train_x, tau)
            loss, mse, kl_loss = gumbel_loss(logits, xhat, train_x, mean, logvar, 1.0, beta=0.3) # gamma
            # loss = tf.reduce_mean(K.losses.mean_squared_error(train_x, xhat))
    ...
    print("Epoch:", epoch, ", TRAIN loss:", loss.numpy(), ", Temperature:", 1.0)
    ...
    if epoch % 1 == 0:
        ...
        for test_x in test_dataset:
            ...
            losses.append(gumbel_loss(logits, xhat, test_x, mean, logvar, 1.0, beta=0.3)[0].numpy()[0])
            # losses.append(K.losses.mean_squared_error(test_x, xhat).numpy()[0])
