...
class GumbelSoftmaxMixtureVAE(K.models.Model):
    ...
    def call(self, x, tau):
        ...
        # logvar = tf.reshape(self.BN(tf.reshape(logvar, (self.params["batch_size"], class_num*latent_dim))),
        #                     (self.params["batch_size"], class_num, latent_dim))
        ...
        # decoder
        xhat = self.decoder(layers.Concatenate(axis=-1)((z_tilde, y))) 
...
def gumbel_loss(logits, xhat, x, y, mean, logvar, temperature, beta):
    # reconstruction
    ...
    kl2 = tf.multiply(tf.reduce_sum(0.5 * (tf.math.pow(mean - PARAMS['prior_means'], 2) - 1 + tf.math.exp(logvar) - logvar), axis=-1), logits_)
    kl2 = tf.reduce_mean(y * kl2)
    kl_loss = kl1 + kl2
...
def center_reconstruction(model, epoch):
    ...
    prior_onehot = tf.cast(np.eye(PARAMS['class_num']), tf.float32)
    center = model.decoder(tf.concat([prior_sample[tf.newaxis, ...], prior_onehot[tf.newaxis, ...]], axis=-1)).numpy()
...
for epoch in range(1, PARAMS["epochs"] + 1):
    ...
    for train_x, train_y in train_dataset:
        with tf.GradientTape(persistent=True) as tape:
            mean, logvar, logits, yhat, z, z_tilde, xhat = model(train_x, tau)
            loss, mse, kl_loss = gumbel_loss(logits, xhat, train_x, yhat, mean, logvar, tau, beta) # gamma
    ...
    if epoch % 10 == 0:
        ...
        example_reconstruction(train_x, yhat, xhat)
    
    if epoch % 1 == 0:
        losses = [] # only ELBO loss
        for test_x, test_y in test_dataset:
            mean, logvar, logits, yhat, z, z_tilde, xhat = model(test_x, tau)
            losses.append(gumbel_loss(logits, xhat, test_x, yhat, mean, logvar, tau, beta)[0].numpy()[0])
...
onehot_ = np.zeros((np.array(grid).shape[0], PARAMS['class_num']))
k = 2
onehot_[:, k] = 1
grid_output = model.decoder(tf.cast(np.concatenate((np.array(grid), onehot_), axis=-1), tf.float32))
...
plt.savefig('./result/mixturevae_mnist_z_200919.png')
#%%
'''t-SNE'''
...
if PARAMS['FashionMNIST']:
    plt.savefig('./result/mixturevae_fashionmnist_tsne_200919.png')
else:
    plt.savefig('./result/mixturevae_mnist_tsne_200919.png')
#%%
r = 6
prior_means = np.array([[r*np.cos(np.pi/10), r*np.sin(np.pi/10)],
                        [r*np.cos(3*np.pi/10), r*np.sin(3*np.pi/10)],
...
plt.savefig('./result/mixturevae_mnist_prior_means_200919.png')
#%%