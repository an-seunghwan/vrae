...
def gumbel_loss(logits, xhat, x, mean, logvar, temperature, beta):
    ...
    kl2 = tf.reduce_mean(tf.multiply(tf.reduce_sum(0.5 * (tf.math.pow(mean - PARAMS['prior_means'], 2) - 1 + tf.math.exp(logvar) - logvar), axis=-1), logits_))
    kl_loss = kl1 + kl2
...
for epoch in range(1, PARAMS["epochs"] + 1):
    ...
    for train_x, train_y in train_dataset:
        with tf.GradientTape(persistent=True) as tape:
            ...
            loss1, mse, kl_loss = gumbel_loss(logits, xhat, train_x, mean, logvar, tau, beta) # gamma
