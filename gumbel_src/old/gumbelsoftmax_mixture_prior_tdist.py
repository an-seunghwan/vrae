...
- MSE + KL + CrossEntropy
- fixed tau
- no weight on CrossEntropy loss
...
class GumbelSoftmaxMixtureVAE(K.models.Model):
    ...
    def sampling_t_dist(self):
        class_num = self.params["class_num"]   
        latent_dim = self.params["latent_dim"]   
        epsilon = tf.random.normal((self.params["batch_size"], class_num, latent_dim))
        chi = tf.tile(tf.cast(np.random.chisquare(self.params['df'], (self.params['batch_size'], self.params['class_num'], 1)), tf.float32), 
                      (1, 1, self.params['latent_dim']))
        z = self.mean + tf.math.exp(self.logvar / 2) * epsilon * tf.math.sqrt(self.params['df'] / chi)
        return z
    ...
    def call(self, x, tau):
        ...
        chi = tf.tile(tf.cast(np.random.chisquare(self.params['df'], (self.params['batch_size'], self.params['class_num'], 1)), tf.float32), 
                      (1, 1, self.params['latent_dim']))
        z = mean + tf.math.exp(logvar / 2) * epsilon * tf.math.sqrt(self.params['df'] / chi)
...
# sce_loss = K.losses.SparseCategoricalCrossentropy(from_logits=False)
# bce_loss = K.losses.BinaryCrossentropy(from_logits=False)
def gumbel_loss(logits, xhat, x, mean, logvar, temperature, beta):
    ...
    for _ in range(PARAMS['M']):
        z = model.sampling_t_dist()
        term1 = -tf.reduce_sum(logvar, axis=-1)/2
        term2 = (-5/2)*tf.math.log(1 + tf.reduce_sum(tf.math.pow(z - mean, 2) / tf.math.exp(logvar) + 1e-20, axis=-1)/3)
        term3 = (5/2)*tf.math.log(1 + tf.reduce_sum(tf.math.pow(z - PARAMS['prior_means'], 2) + 1e-20, axis=-1)/3)
        kl2 += tf.reduce_mean(logits_ * (term1 + term2 + term3), axis=-1) / PARAMS['M']
...
def center_reconstruction(model, epoch):
    ...
    if PARAMS['FashionMNIST']:
        plt.savefig('./result/mixturevae_fashionmnist_tdist_center_epoch{}.png'.format(epoch),
                    bbox_inches="tight", pad_inches=0.1)
    else:
        plt.savefig('./result/mixturevae_mnist_tdist_center_epoch{}.png'.format(epoch),
                    bbox_inches="tight", pad_inches=0.1)
...
def example_reconstruction(train_x, y, xhat):
    ...
    for i in range(0, 15, 3):
        ...
        plt.axis('off')
    if PARAMS['FashionMNIST']:
        plt.savefig('./result/mixturevae_fashionmnist_tdist_rebuilt_epoch{}.png'.format(epoch),
                    bbox_inches="tight", pad_inches=0.1)
    else:
        plt.savefig('./result/mixturevae_mnist_tdist_rebuilt_epoch{}.png'.format(epoch),
                    bbox_inches="tight", pad_inches=0.1)
#%%
# data
if PARAMS['FashionMNIST']:
...
for epoch in range(1, PARAMS["epochs"] + 1):
    ...
    # print("MSE:", mse.numpy(), ", CCE:", loss1.numpy() ,", KL loss:", kl_loss.numpy(), ", beta:", beta)
...
#%%
zmat = model.sampling_t_dist().numpy().reshape(PARAMS['batch_size']*PARAMS['class_num'], PARAMS['latent_dim'])
...
plt.savefig('./result/mixturevae_mnist_tdist_z_200920.png')
#%%
'''t-SNE'''
...
if PARAMS['FashionMNIST']:
    plt.savefig('./result/mixturevae_fashionmnist_tdist_tsne_200920.png')
else:
    plt.savefig('./result/mixturevae_mnist_tdist_tsne_200920.png')
#%%
r = 6
prior_means = np.array([[r*np.cos(np.pi/10), r*np.sin(np.pi/10)],
                        [r*np.cos(3*np.pi/10), r*np.sin(3*np.pi/10)],
...
plt.savefig('./result/mixturevae_mnist_tdist_prior_means_200920.png')
#%%