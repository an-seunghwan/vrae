...
for j in tqdm(range(len(user_val_list))):
    ...
    for timestep in range(1, message_maxlen-1):
        ...
        result_ = [result_[i] + message_numvocab.get(x) + ' ' for i,x in enumerate(pred_num)]
    ...
    true_message = [true_message_[i][:re.search(' <PAD>', true_message_[i]).span()[0]] for i in range(len(true_message_))]
    # pprint(true_message)
    true_message_list.extend(true_message)
    # print('\n=====pred=====')
    result = [result_[i][:re.search(' <eos>', result_[i]).span()[0]] for i in range(len(result_)) if re.search(' <eos>', result_[i])]
    result_list.extend(result)
    # pprint(result)
#%% save
with open('./result/template2message_200815.csv', 'w', encoding='euc-kr', newline='') as f:
    wr = csv.writer(f)
    for k in range(len(true_message_list)): 
        wr.writerow(true_message_list[k] + result_list[k])
#%%