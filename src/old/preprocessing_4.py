...
'''save sas dict'''
import pickle
with open("./body_final/sas_dict.pkl", "wb") as f:
    pickle.dump(sas_dict, f)
...
#     else:
#         # user_df = pd.concat([user_df, sas_dict[n].loc[sas_dict[n]['USER_ID_N'] == user_key].reset_index(drop=True)], axis=1)
...
user_df.to_csv('./data/user_df_sample.csv', encoding='euc_kr')
#%%
'''data scaling
...
for i in tqdm(range(len(user_list))):
    if i == 0:
        total_df = next(user_df_gen)
    else:
        total_df = total_df.append(next(user_df_gen))

# scaling
# scaler.fit_transform(sas_dict.get(0))
#%%
'''
