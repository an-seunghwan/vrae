...
'''decoder'''
# reconstruction y
# w3 = [layers.Dense(100) for _ in range(len(semantic_id))] # overfitting
w3_dim = [20, 100, 20, 100]
w3 = [layers.Dense(w3_dim[j]) for j in range(len(semantic_id))] 
...
def loss_fun(y, y_pred, mean, logvar, beta):
    ...
    kl_loss = tf.zeros(())
    for i in range(len(semantic_id)):
        kl_loss += tf.reduce_mean(tf.reduce_sum(-0.5 * (1 + logvar_pred[i] - tf.math.pow(mean_pred[i], 2) - tf.math.exp(logvar_pred[i])), axis=1), keepdims=True)
        
    return recon_loss, kl_loss, recon_loss + beta * kl_loss
#%%
optimizer = tf.keras.optimizers.Adam(0.0002)
optimizer1 = tf.keras.optimizers.Adam(0.0002)
#%%
epochs = 7270
...
with open('./result/validation_guidance_200811.csv', 'w', encoding='euc-kr', newline='') as f:
    wr = csv.writer(f)
