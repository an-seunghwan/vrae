...
def loss_fun2(x, x_pred, y, y_pred, beta):
    # x reconstruction loss
    x_recon_loss = mse(x, x_pred)
    
    '''do not consider padding'''
        # y reconstruction loss
    y_recon_loss = tf.reduce_mean(scce(y, y_pred), keepdims=True)
...
for epoch in range(epochs):
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        recon_loss, x_recon_loss, y_recon_loss = loss_fun2(x_data, x_pred, y_data, y_pred, beta=1)
    ...
    if epoch % 10 == 0:
        print('({} epoch) VRAE loss: {:.6}'.format(epoch, loss.numpy()[0]))
        print('X RECON loss: {:.6}, Y RECON loss: {:.6}, VALID loss: {:.6}'.format(x_recon_loss.numpy(), y_recon_loss.numpy()[0], valid_loss.numpy()[0]))
#%%
