...
- naive
'''
user_input1 = layers.Input((maxlen, sas_num, max_var_num))
w1 = layers.Dense(1, use_bias=False, activation='tanh') 
# w1 = layers.TimeDistributed(layers.Dense(1, use_bias=False, activation='tanh')) # timedistributed?
# or w1 = variational latent coding
attention = tf.nn.softmax((w1(user_input1)), axis=2)
h = tf.squeeze(tf.matmul(attention, user_input1, transpose_a=True), axis=2)
lstm = layers.LSTM(units, return_sequences=True)
z = lstm(h)

user_input2 = layers.Input((maxlen, len(var_numeric)))
h = layers.Concatenate(axis=-1)((z, user_input2))

y_recon_layer = [layers.Dense(n) for n in target_num]
y_recon = [d(h) for d in y_recon_layer]

trend_model = K.models.Model([user_input1, user_input2], y_recon)
trend_model.summary()
#%%
...
for epoch in range(epochs):
    ...
    with tf.GradientTape(persistent=True) as tape:
        ...
        for i in range(batch_size):
            y_pred = trend_model([x_data1[i], x_data2[i]])
            loss += loss_fun(label_y[i], y_pred) / batch_size
